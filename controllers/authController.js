...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Generate token for reset password
  ...
  try {
    ...
    return next(
      new AppError('There was an error seding the email. Try again later!'),
      500
    );
  }
});
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // Get user from protect in routes
  // 2) Check if POSTed current password is correct

  if (!password) {
    next(new AppError('Please provide the password'), 400);
  }
  if (
    !req.user ||
    !(await req.user.correctPassword(passwordCurrent, user.password))
  ) {
